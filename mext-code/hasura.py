...
def read_by_subject(school: str, subject: str, goal: list[str], limit: int = 10):
    headers = {
        "Content-Type": "application/json",
        "x-hasura-admin-secret": HASURA_SECRET,
    }
    query = """
    query MyQuery($school: bpchar!, $subject: bpchar!, $goal: [bpchar!]!, $limit: Int!) {
        codes (
            where: {
                version: {
                    _eq: "8"
    ...
    """
    variables = {"school": school, "subject": subject, "goal": goal, "limit": limit}
    result = httpx.post(
